...
if not video_cap.isOpened():
    print('Impisible abrir o video: ' + input_video)
...
video_out_quad = cv2.VideoWriter('video_out_quad.avi',
	cv2.VideoWriter_fourcc(*'XVID'), fps, size_quad)

# Funcion paa anotar os frames
...
# Creamos o obxecto para restar o fondo
...
while True:
    ...
    drawBannerText(frame_fg_mask, 'Mascara primeiro plano')
    drawBannerText(frame_fg_mask_erode, 'Mascara primeiro plano erosionada')
    ...
    cv2.imshow('Saida',frame_composite)
    k = cv2.waitKey(1)
    if k == ord('q'):
    	break
