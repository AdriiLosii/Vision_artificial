...
for (int h_i=border; h_i < height + border; h_i++){
    for (int w_i=border; w_i < width + border; w_i++){
        if (demoImage.at<uchar>(h_i-border, w_i-border)){
            cout << "Atopado Pixel branco @ " << h_i << "," << w_i << endl;
	    ...
	    imshow("imaxe",paddedDemoImage*255);
	    waitKey(0);
        }
    }
}

//Recortamos a imaxe padeada para que coincida coas da orixinal
Mat dilatedImage = paddedDemoImage(Range(border,border+height),Range(border,border+width));
imshow("imaxe",dilatedImage*255);
waitKey(0);
...
imshow("imaxe",dilatedEllipseKernel*255);
waitKey(0);
